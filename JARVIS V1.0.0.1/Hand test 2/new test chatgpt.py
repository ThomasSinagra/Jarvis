...
def worker(coordoneey, coordoneex):
    ...
    def fonctionok(hand_landmarks):
        for idx, landmark in enumerate(hand_landmarks.landmark):
            coordoneex[idx]=(landmark.x)
            coordoneey[idx]=(landmark.y)
...
if __name__ == "__main__":
    ...
    # Accès aux coordonnées partagées mises à jour par le processus secondaire
    while True :
        time.sleep(1)
        print("Coordonnées x dans le processus principal :", list(coordoneex))
        print("Coordonnées y dans le processus principal :", list(coordoneey))
